...
`ProxyHttpService.fetch_with_proxies()` 会把代理按健康度排序后**分批竞速**（默认每批 `DEFAULT_PROXY_BATCH_SIZE=5` 个并发），命中即返回。批内线程池用 `with` 按「调用 + 批」即时创建并关闭，不作为成员长期持有 —— 因此不会泄漏，进程退出时也不会被遗留线程 join 拖住。竞速路径走 `HttpService._get()`（单次、不重试），避免单代理失败被重试 3 次、把被放弃的请求拖到 3×timeout。

**可继续优化**:
- 把批大小提升为可配置项（如 `COLLECTOR_PROXY_FETCH_BATCH_SIZE`）
- 或采用优先级队列，进一步减少被放弃的在途请求
