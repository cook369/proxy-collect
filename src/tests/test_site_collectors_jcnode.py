...
def test_get_today_url_extracts_post_link():
    collector = JCNodeCollector(http_client=AsyncMock(spec=HttpClient))
    html = """
    <html><body>
...
@pytest.mark.asyncio
async def test_get_download_tasks_uses_configured_code_without_bruteforce(monkeypatch):
    collector = JCNodeCollector(http_client=AsyncMock(spec=HttpClient))
    ...
    collector.http_client.post = AsyncMock(
        return_value='{"v2ray":"https://example.com/v2ray.txt"}'
    )
    brute_force_password_mock = AsyncMock()
    monkeypatch.setattr(
...
@pytest.mark.asyncio
async def test_get_download_tasks_passes_instance_verifier_to_bruteforce(monkeypatch):
    collector = JCNodeCollector(http_client=AsyncMock(spec=HttpClient))
    collector.today_page = "https://jcnode.com/posts/free-nodes/20260615/"
    collector.verification_code_strategy = DictionaryPasswordStrategy(["1234"])
    collector.skip_if_cached = Mock()
    collector.http_client.post = AsyncMock(return_value="unused")
    ...
    brute_force_password_mock.assert_awaited_once()
    assert (
...
@pytest.mark.asyncio
async def test_verify_code_posts_with_timeout_and_returns_content():
    collector = JCNodeCollector(http_client=AsyncMock(spec=HttpClient))
    collector.http_client.post = AsyncMock(
        return_value='{"v2ray":"https://example.com/v2ray.txt"}'
    )

    result = await collector.verify_code("1234")

    assert result == '{"v2ray":"https://example.com/v2ray.txt"}'
    collector.http_client.post.assert_awaited_once_with(
...
@pytest.mark.asyncio
async def test_verify_code_raises_fatal_error_on_proxy_error():
    collector = JCNodeCollector(http_client=AsyncMock(spec=HttpClient))
    collector.http_client.post = AsyncMock(
    ...
    with pytest.raises(FatalPasswordAttemptError, match="network failed"):
        await collector.verify_code("1234")

    collector.http_client.post.assert_awaited_once()


@pytest.mark.asyncio
async def test_verify_code_rejects_wrong_password():
    collector = JCNodeCollector(http_client=AsyncMock(spec=HttpClient))
    collector.http_client.post = AsyncMock(return_value="口令错误")

    with pytest.raises(ValueError, match="password error"):
        await collector.verify_code("0000")


@pytest.mark.asyncio
async def test_bruteforce_propagates_fatal_attempt_error():
    attempts = []
