...
class TestProxyCacheService:
    ...
    @pytest.mark.asyncio
    async def test_is_valid_expired(self, tmp_path):
        """测试缓存过期"""
        cache_file = tmp_path / "cache.json"
        data = {
        ...
        assert await service.is_valid() is False

    @pytest.mark.asyncio
    async def test_is_valid_not_enough_healthy(self, tmp_path):
        """测试健康代理不足"""
        cache_file = tmp_path / "cache.json"
        data = {
        ...
        assert await service.is_valid(min_health_score=50.0) is False

    @pytest.mark.asyncio
    async def test_get_proxies(self, tmp_path):
        """测试获取健康代理"""
        cache_file = tmp_path / "cache.json"
        service = ProxyCacheService(cache_file)
        ...
        healthy = await service.get_proxies(min_health_score=30.0)
